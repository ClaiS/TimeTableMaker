...
import { TT, DF, getColor, getMonday, addDays, fmtShort, fmtFull, SCHOOL_MAP } from '../shared/data.js'
...
export default function TKBPage({ classes, setClasses }) {
  ...
  classes.forEach(c => { const di = c.thu - 2; if (di >= 0 && di <= 6) dayMap[di].push(c) })

  const saveClass = c => {
    if (c.id && classes.find(x => x.id === c.id)) setClasses(p => p.map(x => x.id === c.id ? c : x))
    else setClasses(p => [...p, c])
    setEditCls(null); setShowAdd(false)
  }
  const deleteClass  = id => { if (window.confirm('Xóa buổi dạy này?')) { setClasses(p => p.filter(c => c.id !== id)); setTooltip(null) } }
  const cancelClass  = id => { setClasses(p => p.map(c => c.id === id ? { ...c, status: 'cancelled' } : c)); setTooltip(null) }
  const restoreClass = id => { setClasses(p => p.map(c => c.id === id ? { ...c, status: 'normal' }    : c)); setTooltip(null) }
  ...
  const schoolsSeen  = [...new Set(classes.filter(c => c.status !== 'cancelled').map(c => c.truong))]
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.topBar}>
        <div style={s.weekNav}>
          <button style={s.navBtn} onClick={() => setWeekOffset(o => o - 1)}>‹</button>
          <div style={{ textAlign: 'center' }}>
            <div style={s.weekRange}>{fmtFull(ws)} – {fmtFull(we)}</div>
            <div style={s.weekSub}>{weekOffset === 0 ? 'Tuần hiện tại' : weekOffset > 0 ? `+${weekOffset} tuần` : `${weekOffset} tuần`}</div>
          </div>
          <button style={s.navBtn} onClick={() => setWeekOffset(o => o + 1)}>›</button>
          <button style={s.todayBtn} onClick={() => setWeekOffset(0)}>Hôm nay</button>
        </div>
        <button style={s.btnAdd} onClick={() => setShowAdd(true)}>+ Thêm buổi</button>
      </div>
      ...
      <div style={s.gridWrap}>
        <div style={{ overflowX: 'auto' }}>
          <table style={s.table}>
            <thead>
              <tr>
                <th style={{ ...s.th, width: 64, background: '#FAFAFA' }}>Tiết</th>
                {DF.map((d, i) => {
                  const dt = addDays(ws, i)
                  const isToday = dt.getTime() === today.getTime()
                  return (
                    <th key={i} style={{ ...s.th, ...(isToday ? s.thToday : {}) }}>
                      <div style={{ fontSize: 12, fontWeight: 700 }}>{d}</div>
                      <div style={{ fontSize: 11, fontWeight: isToday ? 700 : 400, marginTop: 1 }}>{fmtShort(dt)}</div>
                    </th>
                  )
                })}
              </tr>
              ...
              {(() => {
                ...
                for (let t = 1; t <= 15; t++) {
                  ...
                  if (BREAK_BEFORE.has(t)) {
                    rows.push(
                      <tr key={`brk${t}`}>
                        <td style={s.brkTier}><span style={{ fontSize: 8, color: '#B45309' }}>☕</span></td>
                        <td colSpan={7} style={s.brkCell}>
                          {t === 4 ? 'Giải lao 09:00 – 09:20' : 'Giải lao 14:45 – 15:05'}
                        </td>
                      </tr>
                    )
                  }
                  ...
                  for (let d = 0; d < 7; d++) {
                    ...
                    const found = dayMap[d].find(c => c.tb === t)
                    ...
                    if (found) {
                      ...
                      cells.push(
                        <td key={d} rowSpan={rowSpan}
                          style={{ ...s.td, padding: 3, verticalAlign: 'top', position: 'relative' }}>
                          {/*
                      ...
                      cells.push(<td key={d} style={{ ...s.td, background: t % 2 === 0 ? '#fff' : '#FAFAFA' }} />)
                    }
                  }
                  rows.push(<tr key={t} style={{ height: 40 }}>{cells}</tr>)
                }
                return rows
              })()}
            </tbody>
        ...
        <div style={s.legend}>
          {schoolsSeen.map(sk => {
            const cl = SCHOOL_MAP[sk] || { bg: '#F1F5F9', br: '#64748B', tx: '#1E293B' }
            return (
      ...
      {tooltip && (
        <div
          style={{ ...s.tip, left: tooltip.x, top: tooltip.y }}
        >
          {/* Coloured header */}
          ...
          <div style={{ padding: '5px 0' }}>
            {[
              ...
              ['Tiết',       `${tooltip.cls.tb}–${tooltip.cls.tk} (${tooltip.cls.tk - tooltip.cls.tb + 1} tiết)`],
              ['Giờ',        `${TT[tooltip.cls.tb - 1]?.s} – ${TT[tooltip.cls.tk - 1]?.e}`],
              ['Trạng thái', { normal: 'Chính thức', makeup: 'Dạy bù', cancelled: 'Đã hủy' }[tooltip.cls.status]],
      ...
      {/* ── Modals ── */}
      {showAdd && <AEModal onSave={saveClass} onClose={() => setShowAdd(false)} />}
      {editCls && <AEModal init={editCls} onSave={saveClass} onClose={() => setEditCls(null)} />}
    </div>
  )
}
